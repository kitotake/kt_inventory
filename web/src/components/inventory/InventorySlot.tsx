...
const InventorySlot: React.ForwardRefRenderFunction<HTMLDivElement, SlotProps> = (
    ...
    () => ({
      ...
      item: () =>
        isSlotWithItem(item, inventoryType !== InventoryType.SHOP)
          ? { inventory: inventoryType, item: { name: item.name, slot: item.slot }, image: item?.name && `url(${getItemUrl(item) || 'none'}` }
          : null,
      canDrag,
    }),
    [inventoryType, item]
  ...
  useNuiEvent('refreshSlots', (data: { items?: ItemsPayload | ItemsPayload[] }) => {
    ...
    const itemSlot = data.items.find((d) => d.item.slot === item.slot && d.inventory === inventoryId);
    if (!itemSlot) return;
  ...
  const handleClick = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (event.ctrlKey && isSlotWithItem(item) && inventoryType !== 'shop' && inventoryType !== 'crafting')
      onDrop({ item, inventory: inventoryType });
  ...
  return (
    <div
      ...
      style={{
        filter:          !canPurchaseItem(item, { type: inventoryType, groups: inventoryGroups }) || !canCraftItem(item, inventoryType) ? 'brightness(70%) grayscale(100%)' : undefined,
              ...
              <>
                {item?.currency !== 'money' && item.currency !== 'black_money' && item.price > 0 && item.currency ? (
                  <div className="item-slot-currency-wrapper">
                    <img src={item.currency ? getItemUrl(item.currency) : 'none'} alt="currency" style={{ imageRendering: '-webkit-optimize-contrast', height: 'auto', width: '2vh', backfaceVisibility: 'hidden' }} />
                    <p>{item.price.toLocaleString('en-us')}</p>
                  </div>
                ) : (
                  item.price > 0 && (
                    <div className="item-slot-price-wrapper" style={{ color: item.currency === 'money' || !item.currency ? '#22c55e' : '#ef4444' }}>
