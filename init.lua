...
-- https://kitotake.dev/kt_inventory#config

shared = {
    resource = GetCurrentResourceName(),
    framework = GetConvar('inventory:framework', 'union'),
    playerslots = GetConvarInt('inventory:slots', 85),
    playerweight = GetConvarInt('inventory:weight', 60000),
...
if IsDuplicityVersion() then
    ...
    client = {
        ...
        imagepath = GetConvar('inventory:imagepath', 'nui://kt_inventory/web/images'),
...
if not lib then
    return spamError('kt_inventory requires the kt_lib resource, refer to the documentation.')
end

local success, msg = lib.checkDependency('oxmysql', '2.7.3')

if success then
    success, msg = lib.checkDependency('kt_lib', '3.27.0')
end
...
if not LoadResourceFile(shared.resource, 'web/build/index.html') then
    return spamError(
        'UI has not been built, refer to the documentation or download a release build.\n	^3https://kitotake.dev/kt_inventory^0')
end

-- No we're not going to support qtarget any longer.
if shared.target and GetResourceState('kt_target') ~= 'started' then
    shared.target = false
    warn('kt_target is not loaded - it should start before kt_inventory')
end
